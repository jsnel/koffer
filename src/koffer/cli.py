...
def _decrypt_entry_with_migration(koffer: Koffer, key: bytes, entry: SecretEntry) -> tuple[str, bool]:
...
def store_password_in_keyring(password: str) -> bool:
    ...
    try:
        ...
        print(f"Warning: could not store password in credential manager: {e}", file=sys.stderr)
        return False
...
DEFAULT_MAPPINGS: dict[str, tuple[str, str] | tuple[str, str, dict[str, str]]] = {
    ...
    "anthropic-openrouter": ("ANTHROPIC_AUTH_TOKEN", "token", {
...
def cmd_add(args: argparse.Namespace) -> None:
    ...
    companions = {**default_companions, **_parse_companions(getattr(args, "companion", None))}
    ...
    koffer.entries[name] = SecretEntry(
        name=name, env_var=env_var, secret_type=secret_type,
        ciphertext=ciphertext, nonce=nonce, companions=companions
    )
    save_koffer(koffer)
    print(f"Stored '{name}' ({secret_type}) -> ${env_var}", file=sys.stderr)
    if companions:
        print(f"  with companions: {', '.join(f'${k}' for k in companions)}", file=sys.stderr)
...
def cmd_remove(args: argparse.Namespace) -> None:
    ...
    if backed_up:
        print(f"Backup available at {KOFFER_BACKUP_PATH} to undo if needed.", file=sys.stderr)
...
def cmd_unlock(args: argparse.Namespace) -> None:
    ...
    for name, entry in sorted(koffer.entries.items()):
        ...
        try:
            ...
            export_commands.append(format_export(entry.env_var, plaintext, shell, show_full=show_full))
            # Add companion env vars
            for comp_var, comp_value in entry.companions.items():
                export_commands.append(format_export(comp_var, comp_value, shell, show_full=show_full))
        except Exception:
    ...
    if show_full or not obfuscate:
        ...
        try:
            ...
            if shell == "powershell":
                ...
                print("eval \"$(xclip -selection clipboard -o 2>/dev/null || xsel -ob 2>/dev/null || pbpaste 2>/dev/null)\"")
        ...
        if shell == "powershell":
            print(f'[System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String("{encoded}"))|iex')
        else:
...
def cmd_rotate(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        for name, entry in koffer.entries.items():
            try:
                try:
                    decrypted[name] = decrypt_value(old_key, entry.ciphertext, entry.nonce, aad=entry_aad(entry))
                except Exception:
                    decrypted[name] = decrypt_value(old_key, entry.ciphertext, entry.nonce, aad=None)
            except Exception:
                error_exit("incorrect password")
    ...
    if not args.no_keyring:
        ...
        if store_password_in_keyring(new_password):
            print("Master password rotated and updated in credential manager.", file=sys.stderr)
        else:
...
Install with uv:
    ...
    p.add_argument("--shell", "-s", choices=["powershell", "bash"],
    ...
    p.add_argument("--show-keys", "--show", action="store_true",
                   help="Show full keys in output (implies --stdout)")
    p.add_argument("--clear", action="store_true",
                   help="Clear terminal after unlocking")
    ...
    # purge-keyring
    p = sub.add_parser("purge-keyring", help="Remove stored master password from system credential manager")
    ...
    p.add_argument("--names", "-n", nargs="*", metavar="NAME",
